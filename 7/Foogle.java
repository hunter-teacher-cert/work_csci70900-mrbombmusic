...
public class Foogle
{
  ...
  // looks to see if a certain value can be found in an ArrayList and returns the index of that value if it is found
  public static int findElement( ArrayList al, int target ) // takes arguments - ArrayList and value you are looking for
  {
    for( int pos=0; pos<al.size(); pos++) { // for loop to iterate through all elements in ArrayList
      if (al.get(pos).equals(target)) // .get - Returns the element at the specified position in this list.
                                      // .equals - checks for equality
                                      //  if (value in ArrayList at pos is equal to target value...
        return pos;                    // ...return the index of the target value found in the ArrayList
    }
    return -1; // if target value is not found in ArrayList, return -1
  }//end foo()


  //  Creates ArrayList of specified length filled with random values
  // using lo & hi arguments to augment returned random values form Math.random
  public static ArrayList gizmo(int numItems, int lo, int hi)
  {
    ArrayList retArr = new ArrayList(); // initializes new arraylist of unspecified type
    // loops through to add values to array. number of times through specified by numItems argument
    for(int i=0; i<numItems; i++) {
    //  System.out.println(i);  //diagnostic under-the-hood view
    //  retArr.add( Math.random() ); // [0,1) // adds random float between 0 & 1
      // retArr.add( (int)Math.random() ); //0 // adds value of 0
    //  retArr.add( (int)(hi * Math.random()) ); // [0,?) adds random value multiplied by hi argument value
     retArr.add( lo + (int)(hi * Math.random()) ); // [?,?) adds random random value multiplied by hi argument value plus lo argument value
    }
    return retArr;
  }//end gizmo()
  ...
  public static ArrayList wooHa(int numItems, int lo, int hi)
  {
    ...
    // adds value to array - random float btwn 0 -1 multipled by lo argument value coverted to integer
    retArr.add(
               (int)(lo * Math.random())
               );
  ...
  public static void main( String[] args )
  {
    ...
    System.out.println("\n"+"al000"+":");
    ArrayList al000 = gizmo(6,0,1000);
